  ...
  <div class="admin-page">
    ...
    <div v-if="!authed" class="auth-gate">
      ...
      <label class="pill-search">
        ...
        />
        <button class="pill-arrow active" @click="unlock">
          ...
          <div class="card-upload">
            ...
            <label class="upload-label" :class="{ 'has-file': uploadFiles[item.id] }">
              <input
                ...
                accept="image/*"
                class="file-input"
                @change="onFileSelect(item.id, $event)"
              />
              <span v-if="!uploadFiles[item.id] && !item.drawing">
              ...
              <span v-else-if="uploadFiles[item.id]">{{ uploadFiles[item.id]!.name }}</span>
              <span v-else>replace drawing</span>
            </label>
            <button
              v-if="uploadFiles[item.id]"
              class="upload-btn"
              :class="{ loading: uploading[item.id] }"
...
async function unlock() {
  // We'll store the entered secret and test it by hitting the API
  try {
    ...
    // 404 is fine — means auth passed but id not found
  }
...
.upload-label:hover, .upload-label.has-file {
